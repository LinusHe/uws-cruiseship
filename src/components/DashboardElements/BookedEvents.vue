<template>
  <v-container id="bookedEvents" class="event-section">
    ...
    <v-row justify="center">
      <p class="section-name">Booked Events</p>
    </v-row>
            ...
            <v-window v-model="currentPage">
              <v-window-item v-for="page in pages()" :key="page">
                <v-row class="event-container">
                  <EventElement v-for="event in pageEvents()" :key="event.name" :currentEvent="event" @eventDetails="openBooking(event)" type="booked"></EventElement>
                </v-row>
              </v-window-item>
            </v-window>
          </v-tab-item>
...
export default {
  ...
  mounted() {
    Velocity($("#bookedEvents"), { opacity: "0" }, 0);
    Velocity($("#bookedEvents"), { opacity: "1" }, { delay: 400 }, 300);
  },
  ...
  methods: {
    ...
    getAvailableEvents() {
      return available.filter((el) => el.booked == true);
    },
    getDayEvents() {
